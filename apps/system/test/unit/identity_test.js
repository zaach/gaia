...
suite('identity', function() {
  ...
  suite('open popup', function() {
    ...
    test('popup parameters', function() {
      ...
      assert.equal('https://login.persona.org/sign_in#NATIVE', MockTrustedUIManager.mOrigin);
      assert.equal('https://login.persona.org/sign_in#NATIVE', MockTrustedUIManager.mFrame.src);
    });
